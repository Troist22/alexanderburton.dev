/**
 * nav-loader.js — Production Navigation Loader
 * 
 * PURPOSE:
 *   Dynamically load and cache the navigation HTML into the DOM.
 *   Improves performance by caching nav markup in localStorage.
 * 
 * HOW IT WORKS:
 *   1. On page load, checks localStorage for cached nav markup
 *   2. If cached: injects cached HTML immediately (fast)
 *   3. If not cached: fetches nav.html from server, caches it, then injects
 *   4. After injection, calls menu.js listener initialization
 * 
 * CACHE INVALIDATION:
 *   Change NAV_CACHE_VERSION below to force all browsers to refetch nav.html
 *   Example: '3.1' → '3.2' when nav structure changes
 * 
 * PERFORMANCE:
...
(function () {
  // ─────────────────────────────────────────────────────────────
  // CONFIGURATION: Update this when nav.html structure changes
  // ─────────────────────────────────────────────────────────────
  const NAV_CACHE_VERSION = '4.1';
  const NAV_FILE_NAME = 'nav.html';
  const STORAGE_KEY = `navbar_html_cache_v${NAV_CACHE_VERSION}`;

  /**
   * Loads nav.html, caches it, and injects into DOM
  ...
  async function loadNavigation() {
    try {
      ...
      // STEP 4: Initialize menu event listeners (from menu.js)
